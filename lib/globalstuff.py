...
class globalstuff:
    ...
    '''
    def show_info(self):
        infos = ['Artile Little Tool', 'Version: 1.0.5', 'Author: George.Chang', 'Release Date: 2025/05/04']
        for info in infos:
    ...
    def unzip_file(self,path,name):
        ...
        # 解壓縮文件
        self.send_log(f"Unzipping file: {zip_filename} to {extract_to_directory}", level="info", color=Color.YELLOW)
    ...
    def choose_equipment(self):
        self.send_log("Choose your equipment:", level="info", color=Color.YELLOW)
        options = [e.value for e in Equipment]
        for index, option in enumerate(options):
            self.send_log(f"{index + 1}. {option}", level="info", color=Color.YELLOW)
        choice = input("Enter the number of your choice: ")
        try:
            choice = int(choice) - 1
            if 0 <= choice < len(options):
                selected_equipment = options[choice]
                self.send_log(f"You selected: {selected_equipment}", level="info", color=Color.GREEN)
    ...
    def choose_file(self, path, target_type=[]):
        ...
        if len(files) == 0:
            ...
            return None
        self.send_log("Choose a file:", level="info", color=Color.YELLOW)
